...
async function loadReviews() {
  try {
    const res = await fetch("/api/review");   // FIXED ENDPOINT
    const j = await res.json();
    REVIEWS = j.reviews || j || [];
    renderTestimonials();
  } catch (e) {
...
function renderTopContractors() {
  ...
  use.forEach(c => {
    ...
    row.innerHTML = `
      <strong>${c.company || c.name || "Contractor"}</strong>
      <div class="muted">${c.service || ""}</div>
    `;

    row.addEventListener("click", () =>
      window.open(`/c/${c.id}`, "_blank")
    );
...
function renderTestimonials() {
  ...
  (REVIEWS || [])
    ...
    .forEach(r => {
      ...
      node.innerHTML = `
        <strong>${r.name || r.reviewer_name || ""}</strong>
        <div class="muted">${r.rating || 0} ★</div>
        <div>${r.review || r.comment || ""}</div>
      `;
...
function onServiceClick(s) {
  ...
  setTimeout(() => {
    ...
    window.dispatchEvent(
      new CustomEvent("omni-service-selected", { detail: s })
    );
  }, 220);
}
...
function wireButtons() {
  const b1 = $("#btnContractor");
  if (b1) b1.addEventListener("click", () => (location.href = "/contractor-dashboard.html"));

  const b2 = $("#btnAdmin");
  if (b2) b2.addEventListener("click", () => (location.href = "/admin-dashboard.html"));

  const b3 = $("#btnReviews");
  if (b3) b3.addEventListener("click", () => (location.href = "/reviews.html"));

  const b4 = $("#topContractorsBtn");
  if (b4)
  ...
  if (reqBtn)
    reqBtn.addEventListener("click", async () => {
      ...
      if (!payload.name || !payload.phone || !payload.service)
        return alert("Please provide name, phone and service.");
      ...
      if (j.ok) alert("Request sent. Thank you.");
